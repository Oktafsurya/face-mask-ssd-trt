...
class cctvSSD(object):
    ...
    def get_ori_frame(self):
        ...
        url = 'http://localhost:9473/frame-result'
        ...
        url = 'http://localhost:9473/analytics'
        res = requests.get(url, stream=True)
        buffer = b''
        header = b'--data'
        read_data = False # flag for reading the data

        for chunk in res.iter_content(chunk_size=65536):
            if header in chunk:
                read_data = False
                if buffer != b'':
                    data = json.loads(buffer.decode('utf-8')) # Here is the data
                    counter = data['data']['counter']
                    json_data = {'class_str': data['data']['class_str'],
                                    'score': data['data']['score']}
                    res.close()
                    break

            if ((read_data) and (chunk != b'\r\n')):
                buffer += chunk
...
# external_stylesheets = ['/home/okta/Final_Project/Skripsi_Code/dash_practice/dash-object-detection/assets/base.css']
...
def select_footage(dropdown_value):
    ...
    if dropdown_value=='1':
        return html.Img(src="/cctv_yolo", style={'height':'466px','width':'800px', 'margin-top':'0px', 'margin-left':'0px', 'margin-right':'0px', 'margin-bottom':'0px'})
    elif dropdown_value=='2':
        return html.Img(src="/cctv_yolo", style={'height':'466px','width':'800px', 'margin-top':'0px', 'margin-left':'0px', 'margin-right':'0px', 'margin-bottom':'0px'})

    # return html.Img(src="/cctv_yolo", style={'height':'466px','width':'800px', 'margin-top':'0px', 'margin-left':'0px', 'margin-right':'0px', 'margin-bottom':'0px'})
    # return html.Img(src="/cctv_yolo")
...
def update_score_bar(n_intervals):
    ...
    if n_intervals > 0 and not frame_df.empty:
        ...
        print('[INFO] objects:', objects)
        object_count_dict = {x: 0 for x in set(objects)} 

        print('[INFO] object_count_dict:', object_count_dict)
        ...
        print('[INFO] objects_wc:', objects_wc)

        colors = list('rgb(81, 11, 146)' for i in range(len(objects_wc)))

        # Add text information
        y_text = ["{}% confidence".format(round(float(value) * 100)) for value in frame_df['score'].tolist()]
        print('[INFO] y_text:', y_text)

        y_data_plot = [float(sc_val) for sc_val in frame_df["score"].tolist()]
    ...
    if n_intervals > 0 and not frame_df.empty:
        ...
        for y_cord in range(int(root_round)):
            for x_cord in range(int(root_round)):
                ...
                print('[INFO] score_matrix[y_cord][x_cord]:', score_matrix[y_cord][x_cord])
                print('[INFO] type(score_matrix[y_cord][x_cord]):', type(score_matrix[y_cord][x_cord]))
                if float(score_matrix[y_cord][x_cord]) > 0:
...
if __name__ == '__main__':
    ...
    app.run_server(port=8200, debug=False)